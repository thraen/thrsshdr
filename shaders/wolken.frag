...
vec4 map( in vec3 p ){
	float d = 0.2 - p.y;
	vec3 q = p - vec3(1.0,0.1,0.0)* 0.0005*float(_elapsed_t);
	float f;
	f  = 0.5000*noise( q ); q = q*2.02;
	f += 0.2500*noise( q ); q = q*2.03;
	f += 0.1250*noise( q ); q = q*2.01;
	f += 0.0625*noise( q ); d += 3.0 * f;
	d = clamp( d, 0.0, 1.0 );
	vec4 res = vec4( d );
...
vec3 iResolution  = vec3( _w, _h, 0 );
vec4 iMouse		  = vec4( 0, 0, 0, 0);
// float iGlobalTime = float(_elapsed_t);
