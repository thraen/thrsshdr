...
//      fprintf(stderr, "cannot open audio device %s (%s)\n", device, snd_strerror(err)); exit(1); }
    ...
    int dir = 0; // putting -1 here results in rates like 47999 
                 // and that changes visuals significantly (why?)
    if ((val = snd_pcm_hw_params_set_rate_near(handle, hw_params, &rate, &dir)) < 0) {
        fprintf(stderr, "cannot set sample rate (%s) direction %d \n", snd_strerror(val), dir); exit(1); }
    ...
    //// it seems like if call to snd_pcm_hw_params does't work
    ////   we don't get return < 0
